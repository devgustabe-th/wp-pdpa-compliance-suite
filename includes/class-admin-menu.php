...
class Admin_Menu {
    ...
    /**
     * Router for the requests page. This is the main callback for the menu page.
     */
    public function requests_page_router() {
        // We no longer need to call the handler here.
    ...
    /**
     * Renders the Settings page by calling the Settings class.
     */
    public function settings_page_html() {
        if ( class_exists( __NAMESPACE__ . '\Settings' ) ) {
            ...
            echo '<div class="wrap"><h1>Error: Settings Class Not Found</h1><p>Please ensure the file `includes/class-settings.php` exists and is included correctly in `class-main.php`.</p></div>';
        }
    }

	/**
	 * Renders the list of all data requests.
	 */
	public function requests_list_page_html() {
        ...
        <table class="wp-list-table widefat striped fixed">
            ...
            <tbody>
                ...
                <?php else : ?>
                    <tr>
                        <td colspan="6"><?php esc_html_e( 'No data requests found yet.', 'wp-pdpa-cs' ); ?></td>
                    </tr>
                <?php endif; ?>
            </tbody>
        </table>
    ...
    /**
	 * Renders the single request view page.
	 */
        ...
        <div class="wp-pdpa-cs-single-view">
            <div class="details-panel">
                ...
                <table class="form-table">
                     <tbody>
                        <tr>
                            <th><?php esc_html_e( 'Requester Name', 'wp-pdpa-cs' ); ?></th>
                            <td><?php echo esc_html( $request->requester_name ); ?></td>
                        </tr>
                        <tr>
                            <th><?php esc_html_e( 'Requester Email', 'wp-pdpa-cs' ); ?></th>
                            <td><a href="mailto:<?php echo esc_attr( $request->requester_email ); ?>"><?php echo esc_html( $request->requester_email ); ?></a></td>
                        </tr>
                        <tr>
                            <th><?php esc_html_e( 'Request Type', 'wp-pdpa-cs' ); ?></th>
                            <td><span class="badge type-<?php echo esc_attr($request->request_type); ?>"><?php echo esc_html( ucfirst($request->request_type) ); ?></span></td>
                        </tr>
                        <tr>
                            <th><?php esc_html_e( 'Date Submitted', 'wp-pdpa-cs' ); ?></th>
                            <td><?php echo esc_html( date_i18n( get_option( 'date_format' ) . ' ' . get_option( 'time_format' ), strtotime( $request->created_at ) ) ); ?></td>
                        </tr>
                         <tr>
                            <th><?php esc_html_e( 'Current Status', 'wp-pdpa-cs' ); ?></th>
                            <td><span class="badge status-<?php echo esc_attr($request->request_status); ?>"><?php echo esc_html( ucfirst( str_replace( '_', ' ', $request->request_status ) ) ); ?></span></td>
                        </tr>
                        <tr>
                            <th><?php esc_html_e( 'Full Details', 'wp-pdpa-cs' ); ?></th>
                            <td><?php echo nl2br( esc_html( $request->request_details ) ); ?></td>
                        </tr>
                        <?php if ( ! empty( $request->attachment_path ) ) : ?>
                            <tr>
                                <th><?php esc_html_e( 'Attached File', 'wp-pdpa-cs' ); ?></th>
                                <td>
                                    <?php
                                    $uploads_dir = wp_get_upload_dir();
                                    $attachment_url = str_replace( $uploads_dir['basedir'], $uploads_dir['baseurl'], $request->attachment_path );
                                    ?>
                                    <a href="<?php echo esc_url( $attachment_url ); ?>" target="_blank" download>
                                        <?php echo esc_html( basename( $request->attachment_path ) ); ?>
                                    </a>
                                </td>
                            </tr>
                        <?php endif; ?>
            ...
            <div class="actions-panel">
                ...
                <form method="post">
                    <p>
                        <label for="request_status"><?php esc_html_e( 'Update Status To:', 'wp-pdpa-cs' ); ?></label>
                        <select name="request_status" id="request_status" class="widefat">
                            <?php
                            $statuses = [ 'new', 'in_progress', 'awaiting_verification', 'completed', 'rejected' ];
                            foreach ( $statuses as $status ) {
                                printf(
                                    '<option value="%s" %s>%s</option>',
                                    esc_attr( $status ),
                                    selected( $request->request_status, $status, false ),
                    ...
                    <p>
                        <label for="admin_notes"><?php esc_html_e( 'Admin Notes (Internal Use Only)', 'wp-pdpa-cs' ); ?></label>
    ...
    public function dashboard_page_html() {
		...
		$new_requests_count = (int) $wpdb->get_var(
			$wpdb->prepare( "SELECT COUNT(request_id) FROM $requests_table WHERE request_status = %s", 'new' )
		);
		...
		<div class="wrap wp-pdpa-cs-wrap">
			...
			<div class="wp-pdpa-cs-recent-logs">
				...
				<table class="wp-list-table widefat striped">
                    <thead>
						<tr>
							<th><?php esc_html_e( 'User', 'wp-pdpa-cs' ); ?></th>
							<th><?php esc_html_e( 'Consent Type', 'wp-pdpa-cs' ); ?></th>
							<th><?php esc_html_e( 'Details', 'wp-pdpa-cs' ); ?></th>
							<th><?php esc_html_e( 'Date', 'wp-pdpa-cs' ); ?></th>
						</tr>
					</thead>
                    <tbody>
						...
						<?php else : ?>
							<tr>
								<td colspan="4"><?php esc_html_e( 'No consent logs found yet.', 'wp-pdpa-cs' ); ?></td>
							</tr>
						<?php endif; ?>
					</tbody>
				</table>
